...
import {
  getCardioWorkoutScores,
  getIntervalWorkoutCount,
...
async function processCardioWorkout(
  attachment: any,
  workoutName: WorkoutName,
  discordId: string,
  recordDate: "today" | "yesterday"
) {
  ...
  const [minuteStr, heartRateStr] = ocrContent
    .split(", ")
  ...
  const exerciseIntensityZone = determineExerciseIntensityZone(
    heartRate,
    HRM,
    restingHeartRate
  );
  ...
  const createdAt =
    recordDate === "today"
      ...
      : getCurrentDateWithTimezoneOffset(-1);
  ...
  const [imageBuffer, { scoresByWorkout, totalScore }, intervalWorkoutCount] =
    await Promise.all([
  ...
  return {
    files: [
      new AttachmentBuilder(chartImageBuffer!),
