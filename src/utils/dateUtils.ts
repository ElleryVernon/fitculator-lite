export function getCurrentDateUTC(timezoneOffsetHours: number = 9): Date {
  ...
  const adjustedDate = new Date(
    utcMilliseconds + timezoneOffsetHours * 3600000
  );
  return adjustedDate;
}
...
export function getStartAndEndOfWeek(
  ...
  if (isLastWeek) {
    adjustedDate.setUTCDate(adjustedDate.getUTCDate() - 7);
  }
...
export function getCurrentDateWithTimezoneOffset(
  daysOffset: number = 0,
  timezoneOffsetHours: number = 9
): Date {
  const now = new Date();
