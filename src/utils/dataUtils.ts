...
export async function getAllWorkoutData(): Promise<UserData[]> {
  ...
  const summedPoints: UserData[] = usersWithWorkoutPoints
    .map((user) => {
      const cardioWorkouts = user.Workouts.filter(
        (workout) => workout.category === "CARDIO"
      );
      const intervalWorkouts = user.Workouts.filter(
        (workout) => workout.category === "INTERVAL"
      );
      const totalPoints = cardioWorkouts.reduce(
        (acc, workout) => acc + workout.point,
        0
      );
...
export async function getCardioWorkoutScores(
  discordId: string,
  isLastWeek: boolean = false
) {
  const { startOfWeek, endOfWeek } = getStartAndEndOfWeek(isLastWeek);
  ...
  const scoresByWorkout: ScoresByWorkout = cardioWorkouts.reduce(
    (acc: any, workout) => {
  ...
  const totalScore = cardioWorkouts.reduce(
    (sum, workout) => sum + workout.point,
    0
  );
...
export async function getIntervalWorkoutCount(
  discordId: string,
  isLastWeek: boolean = false
) {
  const { startOfWeek, endOfWeek } = getStartAndEndOfWeek(isLastWeek);
...
export async function calculateWeeklyLoadsForUser(discordId: string) {
  ...
  if (
    ...
    userWithSubscription.subscriptions.length === 0
  ) {
    throw new Error("사용자 또는 구독 정보를 찾을 수 없습니다.");
  }

  const latestSubscription = userWithSubscription.subscriptions[0];
...
export async function getWeeklyWorkoutData(
  ...
  workouts.forEach((workout) => {
    const dayIndex = workout.createdAt.getUTCDay();
