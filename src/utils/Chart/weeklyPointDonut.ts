...
function loadPngImage(
  imagePath: string
): Promise<{ img: Image; width: number; height: number }> {
...
function getDonutChartOptions(
  ...
  const doughnutLabel: Plugin<"doughnut"> = {
    ...
    beforeDatasetsDraw(chart, args, pluginOptions) {
      ...
      ctx.font = "semibold 40px NanumGothic";
      ctx.fillStyle = "white";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText("유산소 운동", xCoor, yCoor - 84);
      ...
      ctx.font = "semibold 40px NanumGothic";
      ctx.fillText(
        `근력운동 ${options.intervalWorkoutCount}회`,
        xCoor,
        yCoor + 140
      );
